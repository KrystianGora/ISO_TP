...
/*
 * @brief Timeout - waiting for flow frame should be no longer than 7 seconds
 * (70 000 * 0.1 ms)
 */
#define FLOW_FRAME_TIMEOUT       (70000UL)
 ...
 */
static void ISOTP_GetTimeStamp(uint32_t * timeStamp_pu32);
...
void ISOTP_TxStateMachine( void )
{
    ...
    static uint32_t timeStampStart_u32;     /* Resolution: 0.1 ms */
    static uint32_t timeStampStop_u32;      /* Resolution: 0.1 ms */
    static uint8_t frameIndex_u8;
    static uint8_t isFirstConsFrameTranssmited_u8 = RESET;
    uint8_t currentFramePayloadSize_u8;
    uint8_t currentTimeStampOffset_u8;  /* Resolution: 0.1 ms */

    /* Increment value of global Time Stamp */
    TimeStamp_u32++;
    ...
    case SEND_FIRST_FRAME_STATE:
        ...
        /* Start counting time for waiting flow frame timeout*/
    ...
    case WAIT_FOR_FLOW_FRAME_STATE:
        ...
        else
        {
            ...
            ISOTP_GetTimeStamp(&timeStampStop_u32);
            if( timeStampStart_u32 + FLOW_FRAME_TIMEOUT < timeStampStop_u32 )
            {
                /* Report the timeout error */
                CurrentErrorStatus_u8 = ISOTP_FLOW_FRAME_TIMEOUT;
                /* Reset State Machine */
                CurrentState_u8 = IDLE_STATE;
            }
        }
...
ISOTP_ErrorType_t ISOTP_IsTxStateMachineBusy(void)
{
    uint8_t retStatus_u8 = ISOTP_NO_ERROR;
    ...
    if (CurrentState_u8 != IDLE_STATE)
    {
        retStatus_u8 = ISOTP_SENDING_OTHER_MSG_IN_PROGRESS;
    }
